 ...
 *
 * @param {String} key
 * @param {Object} query - keys match to string values for equality, to array for ranges. `{ sex: 'M'}`, or `{ age: [20,30] }`.
...
HttpClient.prototype.query = function(bucket, query, options, callback) {
  ...
  if(field==='$key'){
    ...
    type = typeof value == 'number' ? '_int/' : '_bin/';
  }
  var key= "index/" + field + type + encodeURIComponent(value)
